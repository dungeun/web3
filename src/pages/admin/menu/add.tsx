...
export default function AdminMenuAdd() {
  ...
  const fetchParentMenus = async () => {
    try {
      ...
      setParentMenus(data.filter((menu: any) => !menu.parentId))
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(errorData.message || '메뉴 추가 중 오류가 발생했습니다.')
      }
    } catch (error) {
  ...
  const handleChange = (field: string, value: any) => {
    ...
    // 제목이 변경될 때 자동으로 경로 생성
    if (field === 'title' && value) {
      newFormData.path = generatePathFromTitle(value)
    }
        ...
        <div className="content-header">
          <div className="d-flex justify-content-between align-items-center">
            <div>
              ...
              <p className="text-muted mb-0">새로운 메뉴 항목을 추가하세요</p>
            </div>
